...
class Node : public Nodes {
    public: 
        ...
        friend ostream& operator<<(ostream& out, const Node& node) {
                ...
                out << "Reliability at dispatch : " << node._unchanged_reliabilty << endl;
                out << "Energy : " << node.cost << endl;
                out << "Actual execution time: "<<node.actualexecutionTime<<endl;
                out << "-------------------------------------------------------" << endl;
                return out;
        }
        ...
        void setRealibity(){
            _unchanged_reliabilty = realibilty;
        }
...
void assignEST(FullWorkflow& wf, vector<int>&topsort, vector<VM>&VMs)
{
    ...
    for(int i =0;i<numOfNodes;i++){
        if(i==0 || wf.nodes[topsort[i]].isDispathed) continue;
        wf.nodes[topsort[i]].earliestStartTime = numeric_limits<double>::min();
        wf.nodes[topsort[i]].earliestFinishTime = numeric_limits<double>::min();
    }
    ...
    for(int i=0; i<numOfNodes; i++)
    {
        ...
        {
            if(!wf.nodes[child.first].isDispathed){
...
void DeadlineDistrubution(FullWorkflow &wf, vector<VM> &Vms){
    ...
    for (auto node : wf.nodes){
        ...
        omega+=node.actualexecutionTime/(bestVM.getCompPower()*bestfeq);
    }
    ...
    while(!qu.empty()){
        ...
        double omega_x = 0;
        for(auto node: curr){
            if(!wf.nodes[node].isDispathed)
            omega_x+=wf.nodes[node].worstCase/(bestVM.getCompPower()*bestfeq);
            else
            omega_x+=wf.nodes[node].actualexecutionTime/(bestVM.getCompPower()*bestfeq);
        }
        for(auto node : curr){
...
void schedule(FullWorkflow & wf, vector<VM>&VMs, vector<Node>&workflowNodes, bool use_deadline_distribution=false, bool takeRandomDecision=false){
    ...
    for(auto node : workflowNodes){
        ...
        if(wf.nodes[node_index].isDispathed) continue;
        ...
        if(found != 0)
        {
            ...
            if(wf.nodes[node_index].numOfReplicas==1 && !wf.nodes[node_index].dummy && !wf.nodes[node_index].isDispathed)
            pq.push(node_index);
            ...
            while(!q.empty())
            {
                int idx = q.front();
                q.pop();
                if(wf.nodes[idx].isDispathed) continue;
...
void meetReliability(FullWorkflow &workflow){
    ...
    for (int i =0;i<n;i++){
        if(!workflow.nodes[i].dummy && workflow.nodes[i].numOfReplicas==1 && !workflow.nodes[i].isDispathed)
...
void simulateDynamic(FullWorkflow &wf,vector<VM>& VMs ,pair<bool, vector<int>> &p,int seed =0){
    assignEST(wf, p.second, VMs);
    assignLST(wf, p.second, VMs);
    wf.setPrintingEarliestAndLatestTimes();
    vector<int>indeg(wf.nodes.size(),0);
    for (int i =0;i<wf.nodes.size();i++){
        indeg[i] = wf.nodes[i].parent.size();
        wf.nodes[i].indeg = indeg[i];
    }
    ...
    pq.push({0,0});
    wf.nodes[0].isDispathed = true;
    while(!pq.empty()){
        auto it = pq.top();
        pq.pop();
        int u = it.second;
        // cout<<u<<" "<<it.first<<endl;
        wf.nodes[u].finishTime = it.first;
        wf.nodes[u].earliestFinishTime = it.first;
        wf.nodes[u].latestFinishTime = it.first;
        wf.nodes[u].nodeDeadline = it.first;
        wf.nodes[u].realibilty = 1 - pow (1 - wf.nodes[u].assignedVM.getReliability(wf.nodes[u].assignedFreq,wf.nodes[u].actualexecutionTime),wf.nodes[u].numOfReplicas);
        assignEST(wf, p.second, VMs);
        assignLST(wf, p.second, VMs);
        wf.setPrintingEarliestAndLatestTimes();
        ...
        for (auto child : wf.nodes[u].children){
            int v = child.first;
            indeg[v]--;
            wf.nodes[v].indeg--;
            if (indeg[v]==0){
                wf.nodes[v].startTime = wf.nodes[u].finishTime;
                wf.nodes[v].isDispathed = true;
                wf.nodes[v].setRealibity();
                double randomexec = wf.nodes[v].get_actual_time(seed);
                wf.nodes[v].actualexecutionTime = randomexec;
                double timetaken = randomexec/(wf.nodes[v].assignedFreq*wf.nodes[v].assignedVM.getCompPower());
                wf.nodes[v].cost= wf.nodes[v].assignedVM.getEnergy(randomexec,wf.nodes[v].assignedFreq);
                pq.push({wf.nodes[v].startTime+timetaken,v});
            }
        }
    }
}
...
int main()
{
    ...
    ofstream out("./Graphing/dynamic_WithDeadline"+to_string(rel)+"_mu_"+to_string(mu)+".txt");
    out<<files.size()<<endl;
    for (auto fileName : files){
        long double totalEnergy = 0;
        long double totalReliability = 0;
        long double totalMakespan = 0;
        ...
        // string fileName = "Montage_25.txt";
            ...
            simulateDynamic(wf, VMs, p);

            // printResults(fileName,wf,"dynamic");
            wf.checkDAGConstraint();
            totalEnergy+=(long double)wf.totalEnergy;
            totalReliability+=(long double)wf.totalRel;
            totalMakespan+=(long double)wf.nodes.back().finishTime;
        ...
        out<<totalMakespan/5<<" "<<totalEnergy/5<<" "<<totalReliability/5<<endl;
    }
    }
    return 0;
}